...
- De **lod-API** (`…/api/lod/e/…`, `config=default`, resultaatvorm `resultpage`) is de gecureerde API voor externe ontwikkelaars en wordt als zodanig gedocumenteerd, met `person` (op ARK-IRI — dezelfde als in de linked data), `keyword_objects` en `object` als voorbeelden; `search` blijft op `in10`. De pagina linkt naar de `swagger.json` van beide API's. De same-origin proxy dekt nu beide API's (`/spinque-api/{api}/…`). De client-libraries bouwen alleen `results`/`statistics` en kunnen dus niet met de lod-endpoints praten; die voorbeelden gebruiken gewone HTTP-requests (2026-07-29).
