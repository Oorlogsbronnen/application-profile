...
- De **lod-API** (`…/api/lod/e/…`, `config=default`, resultaatvorm `resultpage`) is de basis en wordt als enige uitgelegd, met `person` (op ARK-IRI — dezelfde als in de linked data), `keyword_objects` en `object` als voorbeelden. `in10` wordt uitgefaseerd; het `search`-voorbeeld gebruikt tijdelijk nog de `in10`-URL zonder verdere duiding — zodra `search` naar lod verhuist, hoeft alleen `in10` door `lod` vervangen te worden in dat voorbeeld. De pagina linkt naar de `swagger.json` van de lod-API. De same-origin proxy dekt beide API's (`/spinque-api/{api}/…`). De client-libraries bouwen alleen `results`/`statistics` en kunnen dus niet met de lod-endpoints praten; die voorbeelden gebruiken gewone HTTP-requests (2026-07-29).
- `ontology/examples/CreativeWorkShape.jsonld` is gevuld met een echt collectie-object (trouwfoto Joods Cultureel Kwartier, `schema:Photograph`), triple-voor-triple geverifieerd tegen het endpoint — de objecten-graaf gebruikt in de praktijk een dc:-vocabulaire naast schema.org, en ook dat tonen we zoals het is (2026-07-29).
